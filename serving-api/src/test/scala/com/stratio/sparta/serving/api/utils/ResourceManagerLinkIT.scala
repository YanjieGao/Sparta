...
class ResourceManagerLinkIT extends FlatSpec with MockFactory with
  ...
  it should "return local Spark UI link" in {
    ...
    serverSocket.socket.bind(new InetSocketAddress("localhost", 4040))
    ...
    ResourceManagerLink.getLink should be(Some("http://localhost:4040"))
  }
